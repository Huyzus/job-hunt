...
`job-hunt` is a collection of Markdown prompts, Node.js scripts, and HTML templates. It is strictly a local execution tool. The maintainers do not host, deploy, or operate an AI system, nor do we provide API access to Large Language Models (LLMs).
...
job-hunt interacts with career portals and job boards (Greenhouse, Lever, Ashby, Workday, and others).
...
job-hunt is designed to help individuals make better career decisions, not to automate away human judgment. Acceptable use includes:
