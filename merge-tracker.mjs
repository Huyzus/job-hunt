 ...
 *
 * Run: node job-hunt/merge-tracker.mjs [--dry-run] [--verify]
 */
...
const JOB_HUNT = dirname(fileURLToPath(import.meta.url));
// Support both layouts: data/applications.md (boilerplate) and applications.md (original)
const APPS_FILE = existsSync(join(JOB_HUNT, 'data/applications.md'))
  ? join(JOB_HUNT, 'data/applications.md')
  : join(JOB_HUNT, 'applications.md');
const ADDITIONS_DIR = join(JOB_HUNT, 'batch/tracker-additions');
const MERGED_DIR = join(ADDITIONS_DIR, 'merged');
const DRY_RUN = process.argv.includes('--dry-run');
const VERIFY = process.argv.includes('--verify');

// Ensure required directories exist (fresh setup)
mkdirSync(join(JOB_HUNT, 'data'), { recursive: true });
...
// Generic seniority/role qualifiers that don't distinguish roles within a company.
// Matching only on these (e.g., "Senior Staff X" vs "Senior Staff Y") is a false positive.
const ROLE_NOISE_WORDS = new Set([
  'senior', 'staff', 'principal', 'lead', 'junior', 'mid',
  'product', 'design', 'designer', 'engineer', 'developer', 'manager',
  'level', 'title', 'role',
]);

function roleFuzzyMatch(a, b) {
  const tokenize = (s) => s.toLowerCase().replace(/[,()/]/g, ' ').split(/\s+/)
    .filter(w => w.length > 3 && !ROLE_NOISE_WORDS.has(w));
  const wordsA = tokenize(a);
  const wordsB = tokenize(b);
  const overlap = wordsA.filter(w => wordsB.some(wb => wb.includes(w) || w.includes(wb)));
  // Require ≥2 distinguishing-word overlaps after stripping seniority/role noise.
...
for (const file of tsvFiles) {
  ...
  if (!duplicate) {
    // Company + role fuzzy match — but skip closed entries (Discarded/Rejected),
    // since those are dead postings that shouldn't block a fresh role at the same company.
    const normCompany = normalizeCompany(addition.company);
    duplicate = existingApps.find(app => {
      if (normalizeCompany(app.company) !== normCompany) return false;
      const closedStatus = /^(discarded|rejected|skip)$/i.test(app.status?.replace(/\*\*/g, '').trim() || '');
...
if (VERIFY && !DRY_RUN) {
  ...
  try {
    execFileSync('node', [join(JOB_HUNT, 'verify-pipeline.mjs')], { stdio: 'inherit' });
  } catch (e) {
    process.exit(1);
  }
}
