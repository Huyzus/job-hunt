...
- **BambooHR**: list `https://{company}.bamboohr.com/careers/list`; detail of an offer `https://{company}.bamboohr.com/careers/{id}/detail`
...
The levels are additive — all are executed, results are mixed and deduplicated.
...
7.5. **Verify liveness of WebSearch results (Level 3)** — BEFORE adding to pipeline:
   ...
   WebSearch results may be outdated (Google caches results for weeks or months). To avoid evaluating expired offers, verify each new Level 3 URL with Playwright. Levels 1 and 2 are inherently real-time and do not require this verification.

   For each new Level 3 URL (sequential — NEVER parallel Playwright):
   a. `browser_navigate` to URL
   b. `browser_snapshot` to read content
   c. Classify:
      - **Active**: visible job title + role description + visible Apply/Submit/Solicitar control within main content. Do not count generic header/navbar/footer text.
   ...
   **Do not interrupt the entire scan if a URL fails.** If `browser_navigate` gives an error (timeout, 403, etc.), mark as `skipped_expired` and continue with the next one.
...
Each company in `tracked_companies` must have a `careers_url` — the direct URL to its job page. This avoids searching for it every time.
...
Fallback: if you only have the direct ATS URL, first navigate to the company's website and locate its corporate job page. Use the direct ATS URL only if the company does not have its own corporate page.

**Platform known patterns:**
