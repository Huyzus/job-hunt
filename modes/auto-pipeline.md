...
3. **WebSearch (last resort):** Search for role title + company in secondary portals that index the JD in static HTML.
...
Execute exactly the same as `evaluate` mode (read `modes/evaluate.md` for all blocks A-F + Block G Posting Legitimacy).
