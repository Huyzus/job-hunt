...
Processes job offer URLs accumulated in `data/pipeline.md`. The user adds URLs at any time and then executes `/job-hunt pipeline` to process them all.
...
```
If there is a desync, warn user before continuing.
