#!/usr/bin/env node
/**
 * verify-pipeline.mjs — Health check for job-hunt pipeline integrity
 *
 * Checks:
 ...
 *
 * Run: node job-hunt/verify-pipeline.mjs
 */
...
const REPORTS_DIR = join(JOB_HUNT, 'reports');
const STATES_FILE = existsSync(join(JOB_HUNT, 'templates/states.yml'))
  ? join(JOB_HUNT, 'templates/states.yml')
  : join(JOB_HUNT, 'states.yml');
...
for (const e of entries) {
  ...
  if (!match) continue;
  const reportPath = join(JOB_HUNT, match[1]);
