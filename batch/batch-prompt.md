# job-hunt Batch Worker — Full Evaluation + PDF + Tracker Line
...
**RULE: For article metrics, article-digest.md prevails over cv.md.** cv.md may have older numbers — this is normal.
...
**Transversal advantage**: Frame profile as a **"Technical builder"** who adapts their framing to the role:
...
Read `cv.md`. Table with each JD requirement mapped to exact lines from the CV or keys from i18n.ts.

**Adapted to archetype:**
- FDE → prioritize fast and client-facing delivery
...
**PDF:** job-hunt/output/cv-candidate-{company-slug}-{{DATE}}.pdf
**Batch ID:** {{ID}}
...
Upon finishing, print a JSON summary to stdout for the orchestrator to parse:
