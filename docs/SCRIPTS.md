...
Validates that the job-hunt setup is internally consistent: `cv.md` exists and is not too short, `config/profile.yml` exists with required fields, no hardcoded metrics in `modes/_shared.md` or `batch/batch-prompt.md`, and `article-digest.md` freshness (warns if older than 30 days).
...
Checks whether a newer version of job-hunt is available upstream. Outputs JSON to stdout:
